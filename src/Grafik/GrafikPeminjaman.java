...
public class GrafikPeminjaman extends javax.swing.JFrame {

    public GrafikPeminjaman() {
        initComponents();
        chart.setTitle("Chart Data");
        chart.addLegend("Jumlah Peminjaman", Color.decode("#7b4397"), Color.decode("#dc2430"));
        setDataPeminjaman();
    }

    public void setDataPeminjaman() {
        try {
            ...
            if (optionYear == JOptionPane.OK_OPTION) {
                ...
                List<ModelDataPeminjaman> lists = new ArrayList<>();
                DatabaseConnection.getInstance().connectToDatabase();
                String sql = "SELECT \n"
                        + "    EXTRACT(MONTH FROM TANGGAL_PEMINJAMAN) AS month,\n"
                        + "    DATE_FORMAT(TANGGAL_PEMINJAMAN, '%M') AS month_name,\n"
                        + "    COUNT(*) AS \"Jumlah Peminjaman\"\n"
                        + "FROM \n"
                        + "    Peminjaman\n"
                        + "WHERE \n"
                        + "    EXTRACT(YEAR FROM TANGGAL_PEMINJAMAN) = ?\n"
                        + "GROUP BY \n"
                        + "    EXTRACT(MONTH FROM TANGGAL_PEMINJAMAN), month_name\n"
                        + "ORDER BY \n"
                        + "    EXTRACT(YEAR FROM TANGGAL_PEMINJAMAN) DESC, EXTRACT(MONTH FROM TANGGAL_PEMINJAMAN) DESC;";
                ...
                while (r.next()) {
                    ...
                    double amount = r.getDouble("Jumlah Peminjaman");

                    lists.add(new ModelDataPeminjaman(month, amount));
                }
                r.close();
                p.close();
                ...
                if (lists.size() == 1) {
                    ...
                    for (int i = lists.size() - 1; i >= 0; i--) {
                        ModelDataPeminjaman d = lists.get(i);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GrafikPeminjaman.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new GrafikPeminjaman().setVisible(true);
            }
        });
    }
